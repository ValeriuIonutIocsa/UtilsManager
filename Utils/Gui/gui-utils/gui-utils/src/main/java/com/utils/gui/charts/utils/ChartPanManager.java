...
/**
 * ChartPanManager manages drag gestures on an {@link XYChart} by translating them to panning actions on the chart's
 * axes.
 *
...
public class ChartPanManager {
	...
	/**
	 * The default mouse filter for the {@link ChartPanManager} filters events unless only primary mouse button (usually
	 * left) is depressed.
	 */
	private static final EventHandler<MouseEvent> DEFAULT_FILTER = ChartZoomManager.DEFAULT_FILTER;
	...
	/**
	 * Sets the mouse filter for starting the pan action. If the filter consumes the event with Event.consume, then the
	 * event is ignored. If the filter is null, all events are passed through. The default filter is
	 * {@link #DEFAULT_FILTER}.
	 */
	...
	private boolean passesFilter(
		...
		if (mouseFilter == null) {
			...
			passesFilter = !cloned.isConsumed();
		}
		return passesFilter;
	}
	...
	private void startDrag(
		...
		final DefaultChartInputContext context = new DefaultChartInputContext(chartInfo, event.getX(), event.getY());
	...
	private void drag(
		...
		if (dragging) {
			...
			if (panMode == AxisConstraint.BOTH || panMode == AxisConstraint.HORIZONTAL) {
				final double dX = (event.getX() - lastX) / -xAxis.getScale();
				lastX = event.getX();
				xAxis.setAutoRanging(false);
			...
			if (panMode == AxisConstraint.BOTH || panMode == AxisConstraint.VERTICAL) {
				final double dY = (event.getY() - lastY) / -yAxis.getScale();
				lastY = event.getY();
				yAxis.setAutoRanging(false);
