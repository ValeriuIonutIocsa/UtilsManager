...
public final class JFXUtil {
	...
	/**
	 * Find the X coordinate in ancestor's coordinate system that corresponds to the X=0 axis in descendant's coordinate
	 * system.
	 *
	 * @param descendant
	 *            a Node that is a descendant (direct or indirectly) of the ancestor
	 * @param ancestor
	 *            a Node that is an ancestor of descendant
	 */
	@ApiMethod
	...
	/**
	 * Find the Y coordinate in ancestor's coordinate system that corresponds to the Y=0 axis in descendant's coordinate
	 * system.
	 *
	 * @param descendant
	 *            a Node that is a descendant (direct or indirectly) of the ancestor
	...
	/**
	 * Make a best attempt to replace the original component with the replacement, and keep the same position and layout
	 ...
	 *
	 * @param original
	 *            non-null Node whose parent is a {@link Pane}.
	 * @param replacement
	 *            non-null Replacement Node
	 */
	@ApiMethod
	public static void replaceControl(
			final Node original,
			final Node replacement) {

		final Pane parent = (Pane) original.getParent();
	...
	/**
	 * Creates a "Scale Pane", which is a pane that scales as it resizes, instead of reflowing layout like a normal
	 * pane. It can be used to create an effect like a presentation slide. There is no attempt to preserve the aspect
	 * ratio.
	 * <p>
	 * If the region has an explicitly set preferred width and height, those are used unless override is set true.
	 * <p>
	 * If the region already has a parent, the returned pane will replace it via the {@link #replaceControl(Node, Node)}
	 * method. The Region's parent must be a Pane in this case.
	 *
	 * @param region
	 *            non-null Region
	 * @param wParam
	 *            default width, used if the region's width is calculated
	 * @param hParam
	 *            default height, used if the region's height is calculated
	 * @param override
	 *            if true, w,h is the region's "100%" size even if the region has an explicit preferred width and height
	 *            set.
	 *
	 * @return the created StackPane, with preferred width and height set based on size determined by w, h, and override
	 *         parameters.
	 */
	@ApiMethod
	public static StackPane createScalePane(
			final Region region,
			final double wParam,
			final double hParam,
		...
		final StackPane ret = new StackPane();
		ret.setPrefWidth(w);
		ret.setPrefHeight(h);
		if (region.getParent() != null) {
			replaceControl(region, ret);
		}

		// Wrap the resizable content in a non-resizable container (Group)
		final Group group = new Group(region);
